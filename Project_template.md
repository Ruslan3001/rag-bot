...
| **4. Local MVP (Текущий спринт)** | **LLM:** OpenAI / YandexGPT<br>**Emb:** `paraphrase-multilingual-MiniLM-L12-v2`<br>**DB:** FAISS in-memory | Нулевая стоимость БД и эмбеддингов. Минимальный TTM для проверки концепции. Работает локально на любом CPU. | Не масштабируется (in-memory). Нет персистентности и ролевой модели. | Идеальный KISS-подход для локальной разработки и демо-прототипирования. |
...
> **Бот:**
> 1. Ищу в контексте упоминания "станции Абая" и "Транспорта Авалона".
> 2. В документе "Транспорт Авалона" указано: "Максимальная глубина заложения — около 50 м. Эта точка находится под пересечением проспекта Абая и улицы Фурманова (станция «Абая»)".
> 3. Следовательно, ответ — Около 50 м.

**Пример 2: Успешный ответ (извлечение фактов)**
> **Вы:** В каком году начато строительство скоростоного трамвая Веги?
> **Бот:**
> 1. Ищу в контексте информацию о начале строительства скоростного трамвая Веги.
> 2. В документе "Трамвайная сеть Веги" указано: "В 1970 году по решению Совета Министров РСФСР в Волгограде было начато строительство нового вида городского электрического транспорта — скоростного трамвая, являющегося разновидностью легкорельсового городского транспорта с повышенной провозной способностью".
> 3. Следовательно, ответ — В 1970 году.


**Пример 3: Честное «Я не знаю» (Защита от галлюцинаций)**
> **Вы:** Какой пароль от базы данных SQL?
> **Бот:**
> 1. Ищу в контексте информацию о "пароле от базы данных SQL".
> 2. В предоставленных документах базы знаний нет упоминаний паролей, баз данных SQL или доступов к ним.
> 3. Следовательно, ответ — Я не знаю.

**Пример 4: Запрос вне предметной области**
> **Вы:** Кто такой Дарт Вейдер?
> **Бот:**
> 1. Ищу в контексте упоминания "Дарт Вейдер".
> 2. В документах базы знаний нет упоминаний о таком персонаже.
> 3. Следовательно, ответ — Я не знаю.